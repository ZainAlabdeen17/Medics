...
void setupServiceLocator() {
  ...
  getIt.registerLazySingleton<AuthRepository>(() => AuthRepository(api: getIt<ApiConsumer>()));
  getIt.registerLazySingleton<PatientRepository>(() => PatientRepository(api: getIt<ApiConsumer>()));

  // 4. تسجيل الـ Cubits
  getIt.registerFactory<UserCubit>(() => UserCubit(getIt<AuthRepository>()));

    getIt.registerLazySingleton<DoctorRepository>(() => DoctorRepository(api: getIt<ApiConsumer>()));
  getIt.registerFactory<DoctorCubit>(() => DoctorCubit(getIt<DoctorRepository>()));
